...
from modules.roi_processing import (
    ...
    refine_rois_with_cnmfe  # Added new ROI refinement function
)
...
def process_file_pair(tif_path, roi_path, output_dir, config, mode, process_id):
    ...
    try:
        ...
        if mode in ["all", "preprocess"]:
            ...
            # Apply preprocessing steps in sequence if enabled
            output_h5 = os.path.join(slice_output_dir, f"{slice_name}_corrected.h5")
            
            # Main photobleaching correction
            ...
            if is_cnmf_method:
                components_path = os.path.join(os.path.dirname(output_dir), f"{slice_name}_cnmf_components.h5")
        ...
        if mode in ["all", "extract", "analyze"] and config["roi_processing"].get("refine_rois", False):
        ...
        if mode in ["all", "extract", "analyze"]:
            ...
            # Skip standard extraction if we already have refined ROIs and traces
            if 'roi_masks' in locals() and 'roi_data' in locals():
        ...
        if mode in ["all", "analyze"]:
            ...
            return {
                ...
                "timing": {
                    ...
                    "refinement": refinement_time, # New timing for ROI refinement
