...
def analyze_fluorescence(
    ...
    baseline_percentile = config.get("baseline_percentile", 8)  # Added parameter for baseline percentile
    ...
    logger.info(f"Using average of first 10 frames as baseline F0")  # Modified message
    ...
    for i in range(n_rois):
        ...
        # Calculate baseline as average of first 10 frames - MODIFIED
        f0_frames = min(10, n_frames)  # Ensure we don't exceed available frames
        baseline = np.mean(corrected_trace[:f0_frames])
        logger.info(f"ROI {i+1} baseline (average of first 10 frames): {baseline:.4f}")
