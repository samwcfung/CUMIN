...
def analyze_fluorescence(
    ...
    baseline_frames = config.get("baseline_frames", [0, 100])  # Changed to use first 100 frames
    ...
    for i in range(n_rois):
        ...
        # Calculate baseline based on first 100 frames, excluding peaks
        baseline = calculate_baseline_excluding_peaks(corrected_trace, baseline_frames, logger)
...
def correct_photobleaching(trace, baseline_frames, logger):
    """
    Correct for photobleaching by using the slope of the first 100 frames, excluding peaks.
    ...
    -------
    numpy.ndarray
        Photobleaching-corrected trace
    """
    try:
        ...
        if np.sum(mask) > 1:  # Need at least 2 points for linear regression
            ...
            if abs(m) < 1e-5:
                logger.info("No significant photobleaching detected. Slope is nearly flat.")
                return trace.copy()
            
            # Apply correction to the entire trace
            x_all = np.arange(len(trace))
            bleaching_trend = m * x_all + b
            
            # Remove the trend and add back the starting point
            corrected_trace = trace - (bleaching_trend - bleaching_trend[0])
            
            logger.info(f"Applied photobleaching correction with slope: {m:.6f}")
            return corrected_trace
        else:
            logger.warning("Not enough non-peak points to estimate photobleaching. Using original trace.")
            return trace.copy()
    
    except Exception as e:
        logger.error(f"Error in photobleaching correction: {str(e)}. Using original trace.")
        return trace.copy()

def calculate_baseline_excluding_peaks(trace, baseline_frames, logger):
    """
    Calculate baseline as the mean of the first 100 frames, excluding peaks.
    ...
    try:
        ...
        # If all frames would be excluded, keep at least half of them
        if not np.any(mask) and len(baseline_window) > 0:
            logger.warning("All baseline frames would be excluded. Using median instead of mean.")
            return np.median(baseline_window)
        
        # Calculate mean of non-peak frames
        if np.sum(mask) > 0:
            baseline = np.mean(baseline_window[mask])
            logger.info(f"Calculated baseline by excluding {len(peaks)} peaks: {baseline:.4f}")
            return baseline
        else:
            # Fallback if something went wrong with the mask
            baseline = np.mean(baseline_window)
            logger.warning(f"Using all baseline frames: {baseline:.4f}")
            return baseline
    
    except Exception as e:
        logger.error(f"Error in baseline calculation: {str(e)}. Using mean of all frames.")
        return np.mean(trace[baseline_frames[0]:baseline_frames[1]+1])
