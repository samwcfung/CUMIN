...
------------------
Implements photobleaching correction methods including CNMF, bi-exponential fit, 
and binomial exponential correction.
...
def correct_photobleaching(
    ...
    if correction_method == "cnmf":
        # Use CNMF for photobleaching correction (CPU only)
...
def cnmf_correction(data, num_components=None, logger=None):
    ...
    try:
        ...
        params = cnmf.params.CNMFParams(
            ...
            dview=None                    # Parallel processing
        )
        ...
        if logger:
            logger.warning("CaImAn not installed, falling back to bi-exponential method")
        return bi_exponential_fit_correction(data, logger=logger)
    except Exception as e:
        if logger:
            logger.warning(f"CNMF correction failed: {str(e)}, falling back to bi-exponential method")
        return bi_exponential_fit_correction(data, logger=logger)
