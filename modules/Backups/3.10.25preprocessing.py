...
and low-pass filtering approaches.
...
def correct_photobleaching(
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        if correction_method == "lowpass_filter":
            meta_group.attrs['cutoff_freq'] = config.get("cutoff_freq", 0.001)
...
def lowpass_filter_correction(data, cutoff_freq=0.001, smoothing_sigma=2.0, logger=None):
    """
    Correct photobleaching using low-pass filtering method.
    ...
    """
    if logger:
        logger.info(f"Applying low-pass filter correction (cutoff_freq={cutoff_freq})")
    
    # First apply median filter to correct bad pixels
    # Apply median filter to each frame
    median_filtered = np.zeros_like(data)
    for i in range(data.shape[0]):
        median_filtered[i] = median_filter(data[i], size=3)
    
    if logger:
        logger.info("Applied median filter to correct bad pixels")
    
    # Apply low-pass filter to capture slow changes (photobleaching, illumination)
    # Design Butterworth low-pass filter
    b, a = signal.butter(2, cutoff_freq, 'low')
    
    # Reshape data for filtering
    n_frames, height, width = data.shape
    reshaped_data = median_filtered.reshape(n_frames, -1)
    
    # Apply filter along time dimension
    lowpass_data = np.zeros_like(reshaped_data)
    ...
    if logger:
        logger.info("Generated low-pass filtered series for photobleaching estimation")
    
    # Subtract low-pass filtered data from median filtered data to remove slow trends
    corrected_data = median_filtered - lowpass_data
    
    # Add back the mean to keep values in a similar range
    global_mean = np.mean(median_filtered)
    corrected_data += global_mean
    
    if logger:
        logger.info(f"Subtracted low-pass trend and added global mean: {global_mean:.4f}")
